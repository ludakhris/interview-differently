...
- [ ] `GET /api/immersive-sessions/:sessionId` — return session with full response list (needed to populate per-question accordion on feedback page)
...
- [ ] **`ImmersiveFeedbackPage` gap** — per-question accordion is empty until `GET /immersive-sessions/:sessionId` endpoint exists and response IDs are fetched on load
